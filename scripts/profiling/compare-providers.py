...
class ProviderComparator:
        ...
        for i in range(iterations):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"   Request {i+1:2d}: {latency:6.0f}ms "
                          f"({tokens} tokens, "
    ...
    def print_comparison(self) -> None:
        ...
        sorted_results = sorted(self.results, key=lambda r: r.median_latency_ms if r.median_latency_ms > 0 else float('inf'))

        for result in sorted_results:
            success_rate = (result.successful / result.iterations) * 100 if result.iterations > 0 else 0

            print(f"{result.provider:<12} | {result.model:<25} | "
                  f"{success_rate:6.1f}% | "
        ...
        if successful_results:
            ...
            print(f"🐌 Slowest (median latency): {slowest.model} "
                  f"({slowest.median_latency_ms:.0f}ms)")

            print(f"🚀 Fastest generation: {fastest_tokens.model} "
                  f"({fastest_tokens.mean_tokens_per_second:.1f} tokens/s)")

            speedup = slowest.median_latency_ms / fastest.median_latency_ms if fastest.median_latency_ms > 0 else 1
