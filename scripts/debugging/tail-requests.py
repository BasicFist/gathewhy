...
class RequestMonitor:
    ...
    def __init__(self, filter_model=None, filter_provider=None, filter_level=None, show_slow_only=False):
    ...
    def tail_file(self, file_path: Path) -> None:
        ...
        print(f"🔍 Filters: model={self.filter_model or 'all'} "
...
def main():
    ...
    parser.add_argument('log_file', type=Path, nargs='?',
    ...
    parser.add_argument('--stats-interval', type=int, default=30,
                       help="Show stats every N seconds (0 to disable)")

    args = parser.parse_args()

    if not args.log_file.exists():
        print(f"❌ Log file not found: {args.log_file}", file=sys.stderr)
        print(f"💡 Hint: Check if LiteLLM is running and logging is configured", file=sys.stderr)
        sys.exit(1)
