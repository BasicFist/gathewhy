...
class TestProviderSchemaValidation:
    ...
    def test_providers_config_has_providers_key(self, providers_config: dict) -> None:
    ...
    def test_provider_config_structure(self, providers_config: dict) -> None:
    ...
    def test_provider_types_are_valid(self, providers_config: dict) -> None:
    ...
    def test_provider_status_values_are_valid(self, providers_config: dict) -> None:
    ...
    def test_base_urls_are_valid_http_urls(self, providers_config: dict) -> None:
    ...
    def test_provider_models_field_is_list_or_missing(self, providers_config: dict) -> None:
...
class TestModelMappingsSchemaValidation:
    ...
    def test_mappings_config_structure(self, mappings_config: dict) -> None:
        """Verify model-mappings.yaml has expected structure."""
        # Should have at least one of these sections
        keys = mappings_config.keys()
        assert any(
            k in keys for k in ["exact_matches", "patterns", "fallback_chains", "capabilities"]
        ), "model-mappings.yaml missing routing configuration sections"

    def test_exact_matches_structure(self, exact_matches: dict) -> None:
        """Verify exact_matches entries have required fields."""
        for model_name, config in exact_matches.items():
            assert isinstance(model_name, str)
            assert isinstance(config, dict)
            assert "provider" in config, f"Exact match '{model_name}' missing provider field"

    def test_exact_match_priority_values(self, exact_matches: dict) -> None:
    ...
    def test_fallback_chains_are_lists(self, fallback_chains: dict) -> None:
    ...
    def test_patterns_are_valid_regex(self, mappings_config: dict) -> None:
    ...
    def test_capabilities_routing_structure(self, capability_routing: dict) -> None:
        """Verify capability-based routing has valid structure."""
        for capability, config in capability_routing.items():
            assert isinstance(config, dict), f"Capability '{capability}' config is not a dict"
...
class TestLiteLLMConfigurationValidation:
    ...
    def test_litellm_config_has_model_list(self, litellm_config: dict) -> None:
        """Verify litellm config has model_list section."""
        assert "model_list" in litellm_config, "litellm-unified.yaml missing model_list"
        assert isinstance(litellm_config["model_list"], list)

    def test_model_list_entries_have_required_fields(self, litellm_config: dict) -> None:
        ...
        for idx, model_entry in enumerate(model_list):
            for field in required_fields:
                assert field in model_entry, f"Model entry {idx} missing required field: {field}"

    def test_litellm_params_have_model_key(self, litellm_config: dict) -> None:
        ...
        for idx, model_entry in enumerate(model_list):
            ...
            assert (
                "model" in litellm_params
            ), f"Model entry {idx} litellm_params missing 'model' key"

    def test_litellm_config_has_router_settings(self, litellm_config: dict) -> None:
...
class TestCrossConfigurationConsistency:
    ...
    def test_providers_in_mappings_exist_in_providers_config(
        self, providers_config: dict, exact_matches: dict
    ) -> None:
    ...
    def test_no_duplicate_exact_match_model_names(self, exact_matches: dict) -> None:
    ...
    def test_fallback_chain_primary_models_referenced(
        self, fallback_chains: dict, exact_matches: dict
    ) -> None:
        ...
        for primary_model in fallback_chains.keys():
            ...
            assert (
                primary_model in exact_matches
...
class TestConfigurationPortReferences:
    ...
    def test_provider_urls_use_valid_ports(self, providers_config: dict) -> None:
    ...
    def test_litellm_url_port_standard(self, litellm_config: dict) -> None:
...
class TestConfigurationHealthChecks:
    ...
    def test_at_least_one_provider_is_active(self, active_providers: dict) -> None:
        """Verify at least one provider is configured as active."""
        assert len(active_providers) > 0, "No active providers configured"

    def test_at_least_one_exact_match_defined(self, exact_matches: dict) -> None:
        """Verify at least one exact match model is defined."""
        assert len(exact_matches) > 0, "No exact match models defined"

    def test_litellm_model_list_not_empty(self, litellm_config: dict) -> None:
        """Verify litellm model list is not empty."""
        model_list = litellm_config.get("model_list", [])
        assert len(model_list) > 0, "LiteLLM model_list is empty"

    def test_provider_model_names_not_empty(self, active_providers: dict) -> None:
