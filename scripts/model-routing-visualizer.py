    ...
    if mappings:
        ...
        if exact_matches:
            ...
            for model, route_info in list(exact_matches.items())[:10]:  # Show first 10
                ...
                fallback_text = f" {fallback_arrow} {fallback}" if fallback and fallback != 'none' else ""
        ...
        if 'patterns' in mappings:
            ...
            for key, value in mappings.items():
                if isinstance(value, list) and value and isinstance(value[0], dict) and 'pattern' in value[0]:
                    patterns = value
                    break
        ...
        if mappings:
            ...
            for model, route_info in exact_matches.items():
                ...
                if fallback != 'none' and fallback:
                    f.write(f'  "{model}" -> "{fallback}" [label="fallback", style=dashed, color=orange];\n')
        
        f.write("}\n")
    
    print(f"_DOT graph generated: {output_file}")
    print(f"  To visualize: dot -Tpng {output_file} -o model-routing.png")
    print(f"  Or use online DOT viewer (e.g., https://dreampuf.github.io/GraphvizOnline/)")
    print()
